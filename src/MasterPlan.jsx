...
export default function MasterPlan(){
 ...
 return <div ref={panel} className={`plan-viewer ${full?'fullscreen':''}`} role={full?'dialog':undefined} aria-modal={full?true:undefined} aria-label="Original site master plan viewer">
  <div className="plan-toolbar"><span className="drawing-name">SITE MASTER PLAN</span><div className="plan-tools"><button aria-label="Zoom out" disabled={view.zoom<=.75} onClick={()=>zoomAt(current.current.zoom-.25)}>Zoom out</button><output aria-label="Zoom level">{Math.round(view.zoom*100)}%</output><button aria-label="Zoom in" disabled={view.zoom>=5} onClick={()=>zoomAt(current.current.zoom+.25)}>Zoom in</button><button onClick={reset} aria-label="Fit plan to view">Reset</button><button onClick={()=>setFull(value=>!value)} aria-label={full?'Exit fullscreen':'View fullscreen'}>{full?'Close':'Full screen'}</button><a href={IMAGE} target="_blank" rel="noreferrer" aria-label="Open master plan image">Image</a></div></div>
