...
try{
 for(const config of [{name:'desktop',width:1440,height:960},{name:'tablet',width:820,height:1180},{name:'mobile',width:390,height:844}]){
  ...
  if(config.name==='mobile'){
   const {checkMobile3D}=await import('./mobile-qa.mjs');
   mobile3d=await checkMobile3D(page,context,requests);
  }
  ...
  if(config.name==='mobile'){
   ...
   mobile3d.releasesRenderer=true;
  }
  if(errors.length || failures.length)throw Error(config.name+' browser failures: '+JSON.stringify({errors,failures}));
  results.push({viewport:config.name,chapters:count,horizontalOverflow:overflow,masterPlan:'Direct 2384×1684 image; no PDF or worker requests',zoom:true,fullscreen:true,backToTop:true,mobile3d,errors,failures});
  await context.close();
 }
